...
getFilters <- function(project, version="v1", base_url="http://paleocore.org/API") {
  require(httr)
  requestURL <- paste(paste(base_url, version, project, "schema/", sep="/"),"format=json", sep="?")
  ...
  examples<-paste0(names(schema$filtering)[1], c("", "__exact","__contains", "__startswith", "__endswith"), "=", "'somevalue'")
